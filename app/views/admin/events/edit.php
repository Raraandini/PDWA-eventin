...
<section class="eventin-container py-8">
    <div class="mb-8">
        ...
        <h1 class="mt-3 font-display text-4xl font-black text-slate-950">Sunting Event</h1>
        <p class="mt-2 text-slate-600"><?= htmlspecialchars($event['judul']) ?></p>
    </div>
    ...
    <div class="grid gap-6 lg:grid-cols-[1fr_360px]">
        <form data-loading-submit action="<?= $baseUrl ?>/admin/event/update/<?= $event['id'] ?>" method="POST" enctype="multipart/form-data" class="premium-card rounded-[2rem] p-6 sm:p-8">
            <div class="grid gap-5 sm:grid-cols-2">
                ...
                <div>
                    <label class="mb-2 block text-sm font-black text-slate-700" for="status">Status</label>
                ...
                <div class="sm:col-span-2">
                    <label class="mb-2 block text-sm font-black text-slate-700" for="banner">Ganti Banner</label>
        ...
        <aside class="space-y-6">
            <div class="bento-card overflow-hidden">
                <div id="banner-preview" class="h-56 bg-slate-950">
                    ...
                    <?php else: ?>
                        <div class="flex h-full items-center justify-center text-lime-300"><i class="fa-solid fa-image text-4xl"></i></div>
                    <?php endif; ?>
                </div>
                <div class="p-5">
                    <p class="font-black text-slate-950">Banner Saat Ini</p>
                    <p class="mt-1 truncate text-sm text-slate-600"><?= htmlspecialchars($event['banner'] ?: 'Tidak ada banner') ?></p>
                </div>
            </div>
            <div class="bento-card p-6">
                <span class="badge-soft">Status Event</span>
                <p class="mt-4 text-sm leading-6 text-slate-600">Open untuk menerima pendaftaran, closed untuk menutup registrasi, finished untuk event selesai.</p>
            </div>
        </aside>
    </div>
</section>
