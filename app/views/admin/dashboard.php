...
<section class="eventin-container py-8">
    <div class="grid gap-6 lg:grid-cols-[1.25fr_.75fr]">
        ...
        <div class="bento-card p-7">
            <p class="text-xs font-black uppercase text-slate-400">Attendance Rate</p>
            <p class="mt-3 text-6xl font-black text-slate-950"><?= $attendancePercent ?>%</p>
            ...
            <p class="mt-4 text-sm font-bold text-slate-500"><?= $totalHadir ?> hadir dari <?= $totalPendaftaran ?> pendaftaran.</p>
        </div>
    </div>
</section>
        ...
        ?>
            <div class="bento-card p-5">
                <div class="flex items-center justify-between">
                    <p class="text-xs font-black uppercase text-slate-400"><?= $stat[0] ?></p>
                    <i class="fa-solid <?= $stat[2] ?> <?= $stat[3] ?>"></i>
                </div>
                <p class="mt-3 text-4xl font-black text-slate-950"><?= $stat[1] ?></p>
                <p class="mt-1 text-xs font-bold text-slate-500"><?= $stat[4] ?></p>
            </div>
        <?php endforeach; ?>
    </div>
</section>
        ...
        <div class="space-y-6">
            <div class="bento-card p-6">
                ...
                <div class="mt-5 grid gap-3">
                    <a href="<?= $baseUrl ?>/admin/events" class="rounded-3xl border border-slate-200 bg-white p-4 font-black text-slate-800 hover:bg-indigo-50"><i class="fa-solid fa-calendar-check mr-2 text-indigo-500"></i>Kelola Event</a>
                    <a href="<?= $baseUrl ?>/admin/peserta" class="rounded-3xl border border-slate-200 bg-white p-4 font-black text-slate-800 hover:bg-emerald-50"><i class="fa-solid fa-users-gear mr-2 text-emerald-500"></i>Tabel Peserta</a>
                    <a href="<?= $baseUrl ?>/admin/scan" class="rounded-3xl border border-slate-200 bg-white p-4 font-black text-slate-800 hover:bg-lime-50"><i class="fa-solid fa-barcode mr-2 text-lime-600"></i>Scanner Kehadiran</a>
                    <a href="<?= $baseUrl ?>/admin/export/attendance" class="rounded-3xl border border-slate-200 bg-white p-4 font-black text-slate-800 hover:bg-orange-50"><i class="fa-solid fa-file-export mr-2 text-orange-400"></i>Export Attendance</a>
                </div>
            </div>
            <div class="bento-card p-6">
                ...
                <div class="mt-5 space-y-4">
                    <?php foreach ([['Registrasi', $totalPendaftaran, '#6366f1'], ['Hadir', $totalHadir, '#10b981'], ['Pending', max(0, $totalPendaftaran - $totalHadir), '#fb923c']] as $bar): ?>
                        ...
                        <div>
                            <div class="mb-2 flex justify-between text-xs font-black uppercase text-slate-500"><span><?= $bar[0] ?></span><span><?= $bar[1] ?></span></div>
                            <div class="h-3 overflow-hidden rounded-full bg-slate-100"><div class="h-full rounded-full w-[<?= $w ?>%] bg-[<?= $bar[2] ?>]"></div></div>
                        </div>
                    <?php endforeach; ?>
                </div>
            </div>
        ...
        <div class="premium-card rounded-[2rem] p-6">
            <div class="mb-5 flex items-center justify-between">
                <div>
                    ...
                    <h2 class="mt-3 font-display text-3xl font-black text-slate-950">Log Aktivitas</h2>
                </div>
            ...
            <?php if (empty($recentLogs)): ?>
                <div class="rounded-[1.75rem] border border-dashed border-slate-300 bg-slate-50 p-10 text-center">
                    ...
                    <h3 class="mt-4 font-display text-xl font-black text-slate-950">Belum Ada Attendance</h3>
                    <p class="mt-2 text-sm text-slate-600">Aktivitas scan terbaru akan muncul di sini.</p>
                </div>
            <?php else: ?>
                <div class="space-y-3">
                    <?php foreach ($recentLogs as $log): ?>
                        ...
                        <div class="rounded-3xl border border-slate-200 bg-white p-4">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    <p class="font-black text-slate-950"><?= htmlspecialchars($log['nama_peserta'] ?? '-') ?></p>
                                    <p class="mt-1 text-xs font-bold text-slate-500"><?= htmlspecialchars($log['nama_event'] ?? '-') ?> / <?= date('d M Y H:i', strtotime($log['waktu_scan'])) ?></p>
                                </div>
