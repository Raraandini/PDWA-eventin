...
<section class="eventin-container py-8">
    <div class="mb-8 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
            ...
            <span class="badge-soft">Participant Table</span>
            <h1 class="mt-3 font-display text-4xl font-black text-slate-950">Tabel Peserta</h1>
            <p class="mt-2 text-slate-600">Cari peserta, filter event, pantau attendance, dan export laporan.</p>
        </div>
        <div class="relative">
    ...
    <form action="<?= $baseUrl ?>/admin/peserta" method="GET" class="premium-card mb-6 rounded-[2rem] p-5">
        <div class="grid gap-3 lg:grid-cols-[1fr_320px_220px_auto]">
            ...
            <select class="input-modern" name="event_id">
            ...
            <div class="flex gap-2">
                ...
                <a href="<?= $baseUrl ?>/admin/peserta" class="btn-secondary">Reset</a>
            </div>
        </div>
    </form>
    ...
    <?php if (empty($registrations)): ?>
        <div class="premium-card rounded-[2rem] p-10 text-center">
            <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-3xl bg-slate-100 text-slate-400"><i class="fa-solid fa-users-slash"></i></div>
            <h3 class="mt-5 font-display text-2xl font-black text-slate-950">Tidak Ada Peserta</h3>
            <p class="mt-2 text-sm text-slate-600">Belum ada pendaftaran yang cocok dengan filter saat ini.</p>
        </div>
    <?php else: ?>
                ...
                <tbody>
                    <?php foreach ($registrations as $reg): ?>
                        <tr class="participant-row" data-status="<?= htmlspecialchars($reg['status_checkin']) ?>">
                            <td><p class="font-black text-slate-950"><?= htmlspecialchars($reg['nama_peserta']) ?></p><p class="mt-1 text-xs text-slate-500"><?= htmlspecialchars($reg['no_hp_peserta'] ?: '-') ?></p></td>
                            <td><?= htmlspecialchars($reg['email_peserta']) ?></td>
                            <td><?= htmlspecialchars($reg['instansi_peserta'] ?: '-') ?></td>
                            <td><p class="font-bold text-slate-700"><?= htmlspecialchars($reg['nama_event']) ?></p><p class="mt-1 text-xs text-slate-500"><?= date('d M Y H:i', strtotime($reg['tanggal_daftar'])) ?></p></td>
                            <td>
                                <?php if ($reg['status_checkin'] === 'hadir'): ?>
                                    <span class="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700">Hadir</span>
                                    <p class="mt-2 text-xs text-slate-500"><?= date('d M Y H:i', strtotime($reg['waktu_checkin'])) ?></p>
                                <?php else: ?>
                                    <span class="rounded-full border border-orange-200 bg-orange-50 px-3 py-1 text-xs font-black text-orange-600">Pending</span>
                                <?php endif; ?>
                            </td>
                            <td class="font-mono font-black text-indigo-600"><?= htmlspecialchars($reg['kode_tiket']) ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        ...
        <div class="mt-6 flex flex-col gap-4 rounded-[2rem] border border-slate-200 bg-white/80 p-4 shadow-soft sm:flex-row sm:items-center sm:justify-between">
            <p class="text-sm font-bold text-slate-500">Menampilkan <?= count($registrations) ?> dari <?= $totalItems ?> data.</p>
...
<script src="<?= $baseUrl ?>/js/pages/admin-peserta.js"></script>
