...
<section class="eventin-container py-8">
    <div class="grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
        ...
        <div class="grid gap-4 sm:grid-cols-3 lg:grid-cols-1">
            <div class="bento-card p-6"><p class="text-xs font-black uppercase text-slate-400">Event Diikuti</p><p class="mt-2 text-4xl font-black text-slate-950"><?= $totalJoined ?></p></div>
            <div class="bento-card p-6"><p class="text-xs font-black uppercase text-slate-400">Sudah Hadir</p><p class="mt-2 text-4xl font-black text-emerald-600"><?= $totalPresent ?></p></div>
            <div class="bento-card p-6"><p class="text-xs font-black uppercase text-slate-400">Menunggu Scan</p><p class="mt-2 text-4xl font-black text-orange-400"><?= $totalPending ?></p></div>
        </div>
    </div>
</section>
    ...
    <div class="grid gap-6 lg:grid-cols-[.75fr_1.25fr]">
        <aside class="space-y-6">
            <div class="bento-card p-6">
                ...
                <?php if ($nextEvent): ?>
                    <h2 class="mt-4 font-display text-2xl font-black text-slate-950"><?= htmlspecialchars($nextEvent['nama_event']) ?></h2>
                    <p class="mt-2 text-sm font-bold text-slate-500"><?= date('d F Y', strtotime($nextEvent['tanggal_event'])) ?> / <?= date('H:i', strtotime($nextEvent['waktu_event'])) ?> WIB</p>
                    <div class="mt-5 rounded-3xl bg-slate-50 p-4 text-sm font-bold text-slate-600"><i class="fa-solid fa-location-dot mr-2 text-emerald-500"></i><?= htmlspecialchars($nextEvent['lokasi']) ?></div>
                    <a href="<?= $baseUrl ?>/ticket/<?= htmlspecialchars($nextEvent['kode_tiket']) ?>" class="btn-primary mt-5 w-full">Buka Tiket</a>
                <?php else: ?>
                    <h2 class="mt-4 font-display text-2xl font-black text-slate-950">Belum ada upcoming event</h2>
                    <p class="mt-2 text-sm text-slate-600">Temukan event baru yang sedang dibuka.</p>
            ...
            <div class="bento-card p-6">
                ...
                <div class="mt-5 grid gap-3">
                    <a href="<?= $baseUrl ?>/#events" class="rounded-3xl border border-slate-200 bg-white p-4 font-black text-slate-800 hover:border-emerald-200 hover:bg-emerald-50"><i class="fa-solid fa-magnifying-glass mr-2 text-emerald-500"></i>Cari event</a>
                    <a href="<?= $baseUrl ?>/profile" class="rounded-3xl border border-slate-200 bg-white p-4 font-black text-slate-800 hover:border-indigo-200 hover:bg-indigo-50"><i class="fa-solid fa-id-card mr-2 text-indigo-500"></i>Edit profil</a>
                    <button type="button" onclick="window.EventinToast && window.EventinToast('Riwayat seminar siap diekspor dari data peserta.', 'info')" class="rounded-3xl border border-slate-200 bg-white p-4 text-left font-black text-slate-800 hover:border-purple-200 hover:bg-purple-50"><i class="fa-solid fa-file-export mr-2 text-purple-500"></i>Export riwayat</button>
                </div>
            </div>
        </aside>
        ...
        <div class="space-y-6">
            <div class="flex items-end justify-between gap-4">
                <div>
                    ...
                    <h2 class="mt-3 font-display text-3xl font-black text-slate-950">Riwayat Pendaftaran</h2>
                </div>
            </div>

            <?php if (empty($pendaftarans)): ?>
                <div class="premium-card rounded-[2rem] p-10 text-center">
                    <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-3xl bg-lime-100 text-lime-800"><i class="fa-solid fa-ticket"></i></div>
                    <h3 class="mt-5 font-display text-2xl font-black text-slate-950">Belum Ada Pendaftaran</h3>
                    <p class="mx-auto mt-2 max-w-md text-sm text-slate-600">Tiket digital dan sertifikat akan muncul setelah kamu mendaftar event.</p>
                        ...
                        <article class="bento-card p-5">
                            <div class="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                                <div class="min-w-0">
                                    ...
                                    <h3 class="mt-3 font-display text-2xl font-black text-slate-950"><?= htmlspecialchars($pendaftaran['nama_event']) ?></h3>
                                    <p class="mt-2 text-sm font-bold text-slate-500"><?= date('d M Y', strtotime($pendaftaran['tanggal_event'])) ?> / <?= date('H:i', strtotime($pendaftaran['waktu_event'])) ?> WIB - <?= htmlspecialchars($pendaftaran['lokasi']) ?></p>
                                </div>
                                <div class="flex flex-col gap-2 sm:flex-row md:flex-col lg:flex-row">
                                    ...
                                    <?php else: ?>
                                        <button disabled class="rounded-3xl border border-slate-200 bg-slate-100 px-5 py-3 font-black text-slate-400"><i class="fa-solid fa-award mr-2"></i>Sertifikat</button>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </article>
